...
class TextRecordsNotifier extends StateNotifier<List<TextRecord>> {
  ...
  Future<void> renameFolder(String oldName, String newName) async {
    ...
    if (oldName.isEmpty ||
        TextManifest.validateFolderName(newNameTrimmed) != null ||
        newPath == oldName ||
    ...
    // 避免两个文件夹被静默合并
    final existing = await TextManifest.getAllFolders();
    ...
    for (final desc in descendants) {
      ...
      await TextManifest.addFolder(newDescPath);
    }
  ...
  Future<void> moveFolder(String sourceName, String targetParent) async {
    ...
    if (targetParentClean.isNotEmpty) {
      await TextManifest.addFolder(targetParentClean);
      // 空文件夹移动不经过 moveRecord，祖先目录不会被自动跟踪，需逐级补齐
      var ancestor = TextManifest.getParentFolderPath(targetParentClean);
      while (ancestor.isNotEmpty) {
        await TextManifest.addFolder(ancestor);
        ancestor = TextManifest.getParentFolderPath(ancestor);
      }
    }
  ...
  Future<void> copyFolder(String sourceName, String targetParent) async {
    ...
    final records = List<TextRecord>.from(await TextManifest.loadRecords());
