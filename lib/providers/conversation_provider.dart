...
class Conversation {
  ...
  /// on re-entry, taking priority over the assistant default / first model.
  ...
  /// 该对话记录模型的 API 模型 ID（绝对身份）。
  ///
  /// 与 [lastUsedModelName] 一起持久化：显示名可能因模型/供应商重命名
  /// 而过期，模型 ID + 供应商名在重命名后仍能解析回同一模型。旧数据
  /// （仅显示名）为 null，回退到显示名匹配。
  String? lastUsedModelId;

  /// 该对话记录模型的供应商名称（绝对身份的一部分）。
  ...
  Map<String, dynamic> toMap() => {
        ...
        if (lastUsedModelId != null && lastUsedModelId!.isNotEmpty)
          'lastUsedModelId': lastUsedModelId,
        if (lastUsedProviderName != null && lastUsedProviderName!.isNotEmpty)
          'lastUsedProviderName': lastUsedProviderName,
  ...
  factory Conversation.fromMap(Map<String, dynamic> map) {
    ...
    final lastUsedIdRaw = map['lastUsedModelId'];
    final lastUsedProviderRaw = map['lastUsedProviderName'];
    ...
    // 临时对话状态（防御性解析，缺省即普通对话）。isTemporary 与
    // temporaryExpiresAt 必须成对出现（toggleTemporary 保证）：
    // 数据损坏产生"isTemporary=true 但无过期时间"时搁置为普通对话，
    // 否则会成为一个永不过期、且让 1 秒 tick 永不停止的记录。
    final isTemporaryRaw = map['isTemporary'];
    DateTime? temporaryExpiresAt;
    final tempExpiresRaw = map['temporaryExpiresAt'];
    if (tempExpiresRaw is String) {
      try {
        temporaryExpiresAt = DateTime.parse(tempExpiresRaw);
      } catch (_) {
    ...
    return Conversation(
      ...
      lastUsedModelId: lastUsedIdRaw is String ? lastUsedIdRaw : null,
      lastUsedProviderName:
          lastUsedProviderRaw is String ? lastUsedProviderRaw : null,
...
/// 稳定的"置顶优先"排序：置顶对话在前、非置顶对话在后，块内严格保持
/// 传入顺序。
///
/// 不能用 [List.sort] 配合"比较器对同组返回 0"实现：Dart 的 `List.sort`
/// 在列表长度超过 33 时使用 dual-pivot quicksort（不稳定排序），等键元素
/// 会被任意重排——这是对话条目较多时列表顺序"自动乱掉"的根源（条目少
/// 于 34 时走稳定的插入排序，故少量条目不出问题）。
///
/// 分区实现 O(n)，天然稳定。
///
/// 这也是存储不变量：`ConversationsNotifier` 的 state 必须始终保持本
/// 函数的结果顺序（显示顺序 == 存储顺序），否则拖拽跨置顶/非置顶边界
/// 时会产生显示层无法表达的隐藏交错顺序，取消置顶或切换助手后乱序才
/// 会暴露。
List<Conversation> pinnedFirstStable(List<Conversation> conversations) {
  final pinned = <Conversation>[];
  final unpinned = <Conversation>[];
  for (final c in conversations) {
    (c.isPinned ? pinned : unpinned).add(c);
  }
...
/// 每秒递增一次（只要存在任意未到期临时对话）。仅倒计时胶囊等需要
/// **周期性刷新**的 UI 监听它，避免按秒重建整个对话列表/聊天页。
final temporaryCountdownTickProvider = StateProvider<int>((ref) => 0);
...
class ConversationsNotifier extends StateNotifier<List<Conversation>> {
  ...
  /// 临时对话的每秒 tick 计时器：存在任意临时对话时运行——刷新
  /// [temporaryCountdownTickProvider]（倒计时 UI）并检查到期自动删除。
  ...
  /// 存在临时对话时启动每秒 tick，全部消失后停止。
  void _syncTemporaryTimer() {
    final hasTemporary = state.any((c) => c.isTemporary);
    if (hasTemporary) {
      if (_temporaryTimer != null) return;
      _temporaryTimer = Timer.periodic(const Duration(seconds: 1), (_) {
        // 防止已入队的回调在 dispose 之后重挂计时器。
        if (!mounted) return;
        _ref.read(temporaryCountdownTickProvider.notifier).state++;
  ...
  Future<void> _checkTemporaryExpiry() async {
    ...
    final expiredIds = expired.map((c) => c.id).toSet();
    state = state.where((c) => !expiredIds.contains(c.id)).toList();
    if (expiredIds.contains(_ref.read(activeConversationIdProvider))) {
  ...
  Future<void> _cleanupTemporaryConversationFiles(Conversation conv) async {
    ...
    try {
      await AttachmentStorage.deleteConversationCompressedImages(conv.id);
    } catch (_) {
      // 非关键清理
    }
  }
