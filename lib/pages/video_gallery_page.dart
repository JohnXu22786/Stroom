...
class _VideoGalleryPageState extends ConsumerState<VideoGalleryPage> {
  /// Tracks which hashes are currently being thumbnailed (prevents duplicate work).
  ...
  Widget build(BuildContext context) {
    ...
    Future<Uint8List?> _loadOrGenerateThumbnail(VideoRecord file) async {
      ...
      // Avoid duplicate generation of the same thumbnail
      if (!_generatingThumbnails.add(file.hash)) {
        return null; // Already being generated by another FutureBuilder
      }
      ...
      try {
        ...
        debugPrint('_loadOrGenerateThumbnail error for ${file.hash}: $e');
        // Silently fail — fallback to icon will be shown
      } finally {
        _generatingThumbnails.remove(file.hash);
      }
      return null;
    }
    ...
    Widget fileThumbnailBuilder(VideoRecord file) {
      return FutureBuilder<Uint8List?>(
        future: _loadOrGenerateThumbnail(file),
