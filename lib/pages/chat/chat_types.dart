...
///
/// When the MCP master switch is off, MCP tool names must not be *counted
/// as enabled* in the UI even if an explicit preference saved them earlier.
/// The saved preference itself is never modified by this function: the
/// runtime enabled set is only persisted when it differs from the saved set
/// (send/toggle), and explicit saved sets are only ever replaced by actual
/// user toggles — so hidden tools come back automatically when they become
/// selectable again.
///
