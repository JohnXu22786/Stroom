...
///
/// Displays a list of images with swipe-to-page navigation.
/// Non-SVG images use [ExtendedImage] with gesture mode.
/// SVG images use [PhotoView.customChild] with [SvgPicture].
///
/// Top-right has two buttons:
/// - **Crop** — opens [ExtendedImageEditorPage] for basic crop/rotate.
/// - **Edit** — opens [ImageEditorPage] for advanced editing (ProImageEditor).
...
class _GalleryViewerPageState extends State<GalleryViewerPage> {
  ...
  /// Guards against re-entrant loading.
  bool _isLoading = false;

  /// Cached image bytes keyed by record id.
  ...
  /// Read image bytes from disk (with simple in-memory cache).
  ...
  /// Build the image widget for a given record at [index].
  Future<Widget> _buildImagePage(int index) async {
    if (index < 0 || index >= widget.images.length) {
      return const Center(child: Text('无效索引'));
    }
    ...
    if (isSvg) {
      ...
      if (bytes == null || bytes.isEmpty) {
        return _buildErrorWidget('无法加载 SVG');
      }
      return _buildSvgPage(bytes);
    } else {
      // For non-SVG, use ExtendedImage with gesture mode.
      // If a file path is available, use .file() for better memory handling.
  ...
  Future<void> _saveEditedImage(
    ...
    try {
      ...
      if (isSaveAs) {
        // Create a new record (keep original)
        ...
        // Overwrite — replace the original
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          // File name (bottom, above page indicator)
          Positioned(
            bottom: 52,
            left: 16,
            right: 16,
