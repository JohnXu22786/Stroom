  ...
  group('ManifestDatabase video records', () {
    ...
    testWidgets('insert and retrieve', (WidgetTester t) async {
      await ManifestDatabase.insertVideoRecord({
        'id': 'vid_1', 'name': 'test_video', 'hash': 'abc123',
        'format': 'mp4', 'createdAt': DateTime.now().toIso8601String(),
        'size': 1024, 'folder': '', 'duration': 5000,
      });
      final records = await ManifestDatabase.getAllVideoRecords();
    ...
    testWidgets('update existing record', (WidgetTester t) async {
      await ManifestDatabase.insertVideoRecord({
        'id': 'vid_2', 'name': 'original', 'hash': 'def456',
        'format': 'mov', 'createdAt': DateTime.now().toIso8601String(),
        'size': 2048, 'folder': '', 'duration': 3000,
      });
      await ManifestDatabase.updateVideoRecord('vid_2', {'name': 'renamed', 'size': 4096});
      final records = await ManifestDatabase.getAllVideoRecords();
    ...
    testWidgets('delete record', (WidgetTester t) async {
      await ManifestDatabase.insertVideoRecord({
        'id': 'vid_3', 'name': 'to_delete', 'hash': 'ghi789',
        'format': 'mp4', 'createdAt': DateTime.now().toIso8601String(),
        'size': 512, 'folder': '', 'duration': 2000,
      });
    ...
    testWidgets('batch delete', (WidgetTester t) async {
      for (var i = 0; i < 3; i++) {
        await ManifestDatabase.insertVideoRecord({
          'id': 'batch_$i', 'name': 'batch_$i', 'hash': 'hash_$i',
          'format': 'mp4', 'createdAt': DateTime.now().toIso8601String(),
          'size': 100, 'folder': '', 'duration': 1000,
        });
      }
  ...
  group('VideoManifest DB operations', () {
    testWidgets('add and load records', (WidgetTester t) async {
      await VideoManifest.addRecord(VideoRecord(
        name: 'test_video', hash: 'hash1', format: 'mp4',
        createdAt: DateTime.now(), size: 1024, duration: 5000,
      ));
    ...
    testWidgets('rename record (DB only)', (WidgetTester t) async {
      await VideoManifest.addRecord(VideoRecord(
        id: 'rename_me', name: 'old_name', hash: 'hren',
        format: 'mp4', createdAt: DateTime.now(), size: 100, duration: 1000,
      ));
    ...
    testWidgets('move record to folder (DB only)', (WidgetTester t) async {
      await VideoManifest.addRecord(VideoRecord(
        id: 'move_me', name: 'movable', hash: 'hmov',
        format: 'mp4', createdAt: DateTime.now(), size: 100,
        folder: '', duration: 1000,
      ));
    ...
    testWidgets('update record (DB only)', (WidgetTester t) async {
      final record = VideoRecord(
        id: 'upd', name: 'before', hash: 'hupd',
        format: 'mp4', createdAt: DateTime.now(), size: 100, duration: 1000,
      );
      await VideoManifest.addRecord(record);
      await VideoManifest.updateRecord(record.copyWith(name: 'after', size: 999));
      final records = await VideoManifest.loadRecords();
